...
// create database connection
	...
	$con=mysqli_connect("localhost","execesca_new","bJK2=][lq_UdqSgeqM","execesca_new");
	...
	<div class="row">
		...
		<div id="booking" class="ui-accordion ui-widget ui-helper-reset" role="tablist">
			...
			<div class="ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom ui-accordion-content-active ui-accordion-content-first" id="ui-id-2" aria-labelledby="ui-id-1" role="tabpanel" aria-hidden="false" style="display: block;float: left;width: 100%;clear: both;">
				...
				<form name="rms-booking" id="rms-booking" method="GET" action="https://executiveescapes.com.au/search-nw4/" style="float: left;width: 100%;clear: both;">
							...
							foreach($final as $singleRoomPoperty){
								...
								echo "</pre>";
									...
									// echo "BookingRangeAvailable====>>>>".$singleRoomPoperty['BookingRangeAvailable']; 
									  ...
									  // get data details for db 
									  
									  
									  $sql = "SELECT * FROM properties where RoomTypeId = '".$RoomTypeId."'";
									  $result = mysqli_query($con, $sql);
												...
												if(mysqli_num_rows($result) < 0){
													...
													$allImage=json_decode($row['Images'],true);
													
													$image=$allImage['Image'][0];

												?>

		<div class="visible wf-cell vc_col-xs-12 property_attribute" data-search=1 data-name="<?php echo $row['Name']; ?>" data-attributes="<?php echo $final_attributes; ?>">

			<article class="post visible dt_portfolio type-dt_portfolio status-publish">

				<div class="post-thumbnail-wrap  ">

					<div class="post-thumbnail"> <a href="<?php echo home_url(); ?>/details/?id=<?php echo $row['RoomTypeId'];?>&A=<?php echo $arrival; ?>&D=<?php echo $departure; ?>&Ad=<?php echo $adults  ?>&C=<?php echo $children; ?>" class="post-thumbnail-rollover" target="">
				...
				<div class="post-entry-content new">
					...
					<h3 class="entry-title"> <a href="<?php echo home_url(); ?>/details/?id=<?php echo $row['RoomTypeId'];?>&A=<?php echo $arrival; ?>&D=<?php echo $departure; ?>&Ad=<?php echo $adults  ?>&C=<?php echo $children; ?>" target="" title="Woodlands Retreat" rel="bookmark"><?php echo $row['Name']; ?></a> </h3>
...
<?php 
		...
		if (in_array("1-bedroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/bedroom.png' alt='1 bedroom'/></span>";
		}
		
		if (in_array("2-bedroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/2-bedroom.png' alt='2 bedroom'/></span>";
		}
		
		if (in_array("3-bedroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/3-bedroom.png' alt='3 bedroom'/></span>";
		}
		
		if (in_array("4-bedroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/4-bedroom.png' alt='4 bedroom'/></span>";
		}
		if (in_array("5-bedroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/5-bedroom.png' alt='5 bedroom'/></span>";
		}
		if (in_array("1-bathroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/bathroom.png' alt='1 bathroom'/></span>";
		}
		if (in_array("2-bathroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/2-bathroom.png' alt='2 bathroom'/></span>";
		}
		if (in_array("3-bathroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/3-bathroom.png' alt='3 bathroom'/></span>";
		}
		if (in_array("4-bathroom", $final_attributes)){
			echo "<span class='iconItem'><img src='". esc_url( get_template_directory_uri() )  ."/images/4-bathroom.png' alt='4 bathroom'/></span>";
		}
		...
		if (in_array("2-car-space", $final_attributes)){
	...
	?>
					...
					<a class="btn btn-outline-info btn-rounded z-depth-0 waves-effect" href="<?php echo home_url(); ?>/details/?id=<?php echo $row['RoomTypeId'];?>&A=<?php echo $arrival; ?>&D=<?php echo $departure; ?>&Ad=<?php echo $adults  ?>&C=<?php echo $children; ?>"  target="">Book Now</a></div> </div>
		...
		<?php 
						...
						if($countHotel==0){
							...
							echo "<p>Sorry we do not have any properties available for your current search criteria. Please change your search criteria or get in touch with us on +61 8 9286 2641 or at <a href='mailto:info@executiveescapes.com.au'>info@executiveescapes.com.au</a> to see if we can assist you further.</p>";
						}
						...
						<h3 style="color:red;" class="note">*Atleast 3 days gap required in between arrival and departure date.</h3>
