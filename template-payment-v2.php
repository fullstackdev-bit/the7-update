...
if(isset($_POST)){ 
			...
			// echo "<pre>";
				// print_r($_POST);
			// echo "</pre>";
			$totalDeposite=$_SESSION['bookingDetail']['TotalDeposit'];
			$totalPrice=$_SESSION['bookingDetail']['TotalPrice'];
		
			
			// check extra feature added
			$checkIndictorRequirement=0;
			
			$description="";
			if(isset($_POST['Pets'])){
				$description=$_POST['Pets']." , ";
			}
			$description.=$_POST['Note']; 
			
			// Getting Extra Additonal values
			$recordDetail=$_POST['record'];
			$additionalPrice=$_POST['additionalPrice'];
			$additionalName=$_POST['additionalName'];
			$additionalSundry=$_POST['additionalSundry'];
			
			$requirements="<Requirements>";
			foreach ($recordDetail as $key => $value){		
				if($value>0){
						$totalAmount=$value*$additionalPrice[$key];
						$totalDeposite=$totalDeposite+($totalAmount/2);
						$totalPrice=$totalPrice+$totalAmount;
						
						$requirements.="<Requirement>";
						$requirements.="<Id>".$key."</Id>";
						$requirements.="<Name>".$additionalName[$key]."</Name>";
						$requirements.="<Quantity>".$value."</Quantity>";
						$requirements.="<Sundry_Id>".$additionalSundry[$key]."</Sundry_Id>";
						$requirements.="<ExcludeAmountFromDeposit>false</ExcludeAmountFromDeposit>";
						$requirements.="<Amount>".$totalAmount."</Amount>";
						$requirements.="</Requirement>";
						$checkIndictorRequirement++;
				}
			}
			$requirements.="</Requirements>";
			// echo "After total deposite::".$totalDeposite;
			// echo "After total price::".$totalPrice;			
					...
					die();
								...
								$xml_quote='<RMSQuoteRQ>
											   ...
											   <Quotes>
												...
												<BookingSourceId>27</BookingSourceId>
								...
								// append requirements detail over here
								if($checkIndictorRequirement>0){
									$xml.=$requirements;
								}
								// append requirements detail end here
			...
			if(isset($response["Error"])){ 
				...
				if($refrenceNo!=""){
					...
					if ($err2) {
						...
						echo "<h1>Thank you for choosing to book your Perth accommodation with Executive Escapes.</h1> <br /> 

								Your booking reference number is #".$refrenceNo." You will receive a confirmation email from us shortly.

								If you have any questions, please don’t hesitate to contact one of our friendly team members on info@executiveescapes.com.au.
